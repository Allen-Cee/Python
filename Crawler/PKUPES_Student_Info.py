...
phheaders={
	...
	"Connection":"keep-alive",
	"Host":"b.edooon.com",
	"Referer":"http://b.edooon.com/school/pku/index.jsp",
	"Upgrade-Insecure-Requests":"1",
...
while i<1300020000:
	...
	while len(name)<4:
		name="："+name
	phurl="http://b.edooon.com/school/pku/config.jsp"
	phreq=urllib.request.Request(url=phurl,headers=phheaders)
	gphpage=opener.open(phreq).read()
	phpage=gzip.decompress(gphpage).decode("utf-8")
	phsoup=BeautifulSoup(phpage,"lxml")
	phone=phsoup.find_all("input",id="phoneIpt")[0]["value"]
	if(len(phone)!=11):
		phone="-----------"
	print("PROCESSING CURRENT OBJECT NAME "+name+" ALL COUNTED "+str(num)+"\n")
	with open("PKUPES_Student_Info_2013.txt","a+") as f:
		f.write("{0}  {1}{2} {3:>4}   {4}   {5}   {6}\n".format(info[5].string,info[3].string,info[4].string,name,info[1].string,phone,info[2].string))